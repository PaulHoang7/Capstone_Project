"""Training script for XTTS-FT + CTC v3 (deeper 5-layer head). Phase B Track B1.

v3 changes vs v2 (current baseline 25%→36.7% alignment per memory):
  - CTC head: 5-layer MLP (D→D→D→D/2→D/4→V) — 2.78M params (1.71x v2)
  - λ_ctc: 0.5 → 0.7 (stronger alignment pressure)
  - CTC warmup: 2000 → 3500 steps (smoother ramp with higher λ)
  - max_steps: 30000 → 50000 (longer convergence)
  - Output: experiments/xtts_vieneu_ctc_v3 (separate from v2 — preserve baseline)

Goal: improve alignment >50%, which should reduce mid-sentence drift on
comic bubbles where v2 still loses words (e.g., 'diều' instead of 'điều').

Usage:
    conda run -n xtts_env --no-capture-output python train_xtts_ctc_v3.py
    # OR with tmux for long-running (recommended for 50K-step run):
    #   tmux new -s ctc_v3
    #   conda activate xtts_env && python train_xtts_ctc_v3.py 2>&1 | tee /tmp/ctc_v3.log
"""
...
OUT_PATH        = "/mnt/nfs-data/tin_dataset/experiments/xtts_vieneu_ctc_v3"   # iter v3: 5-layer head + λ=0.7
LANGUAGE        = "vi"
CTC_HEAD_VERSION = "v3"          # iter v3: deeper 5-layer head (see ctc_aux_head.py CTCAlignmentHeadV3)

# Training hyperparameters (Phase B Track B1)
BATCH_SIZE       = 4
GRAD_ACUMM_STEPS = 64
LR               = 1e-5
LAMBDA_CTC       = 0.7          # iter v3: stronger CTC weight (was 0.5)
CTC_WARMUP_STEPS = 3500         # iter v3: longer warmup (was 2000) — needed for higher λ
EVAL_EVERY       = 5000
SAVE_EVERY       = 5000
MAX_STEPS        = 50000        # iter v3: longer convergence (was 30000)
...
class GPTTrainerCTC(GPTTrainer):
    ...
    def __init__(self, config: GPTTrainerConfig, lambda_ctc: float = LAMBDA_CTC):
        ...
        # Swap xtts.gpt for XttsGPTWithCTC with same hyperparameters + v3 head
        ...
        new_gpt = XttsGPTWithCTC(
            **self._gpt_kwargs_from_old(old_gpt),
            ...
            ctc_head_version=CTC_HEAD_VERSION,
        )
...
def main():
    ...
    print(f"[ctc-trainer-v3] head={CTC_HEAD_VERSION}, λ_ctc={LAMBDA_CTC}, "
          f"warmup={CTC_WARMUP_STEPS} steps, max_steps={MAX_STEPS}")
